...
- **角色管理**：系统角色（超级管理员、普通用户、管理员）+ 自定义角色
- **权限分配**：树状勾选界面，按角色配置菜单访问权限
- **权限继承**：勾选父菜单自动拥有所有子菜单功能
- **动态菜单**：根据当前登录用户角色自动渲染侧边栏，无权限页面返回 403
- **超级管理员**：拥有全部权限，可管理所有功能
- **管理员**：拥有除「用户权限」和「系统配置」外的所有后台管理权限
...
- **定时自动爬取**：支持立刻采集 / 北京时间定时 / 倒计时三种模式，拨动开关切换，最小单位分钟，最大单位年份，到达时间后自动执行并播放提示音
- **预置**：百度新闻数据源
...
仿微信即时通讯，支持私聊、群聊、文件传输，集成数字员工、手势交互、AI 语音朗读与语音消息。
...
- 文件上传下载、图片预览
- **手势交互**：基于 MediaPipe 的手势识别，5 种手势（张开手掌/食指/食指中指/V字/握拳）触发页面导航、弹窗关闭等操作，1 秒防抖间隔
- **AI 语音朗读**：Web Speech API TTS 中文语音合成，右键文字消息 → 语音朗读，朗读中气泡蓝色高亮
- **语音消息**：点击麦克风切换到语音录制模式（按住说话/松开发送，最长 60 秒），或输入文字后点击喇叭图标 AI 朗读并发送，语音气泡展示喇叭图标 + 时长
- **新消息提示音**：收到私聊消息、群聊消息、好友请求时播放"叮"声提醒
