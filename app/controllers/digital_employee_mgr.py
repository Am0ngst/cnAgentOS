...
class DigitalEmployeeDispatchHandler(AdminBaseHandler):
    ...
    def post(self):
        ...
        if emp["emp_type"] == "AI":
            return self._handle_ai(emp, message)
        else:
            return self._handle_api(emp, message)

    def _handle_ai(self, emp, message):
    ...
    def get(self):
        ...
        try:
            ...
            for chunk in stream:
                ...
                self.write("data: " + json.dumps({"chunk": chunk_text}, ensure_ascii=False) + "\n\n")
                yield self.flush()
